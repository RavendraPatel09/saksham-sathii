...
import { simplifyTextDocument } from '@/lib/api/accommodations';
...
export const SimplifyDocument = () => {
  ...
  const handleSimplify = async () => {
    ...
    try {
      const data = await simplifyTextDocument(inputText);
      setSimplified(data.simplifiedText);
    } catch (err: any) {
      toast.error(err.message || 'Failed to simplify document. Using local mock.');
