...
import { submitInterviewAnswer } from '@/lib/api/assessments';
...
export const Interview = () => {
  ...
  const [feedback, setFeedback] = useState<any>({
    feedbackConfidence: 85,
    feedbackCommunication: 72,
    feedbackClarity: 90,
    feedbackTechKnowledge: 60,
    feedbackSuggestions: [
  ...
  const handleStop = async () => {
    ...
    try {
      const data = await submitInterviewAnswer({
        ...
        answer: answer || 'Yes, I am experienced in accessibility testing and WCAG Level AA requirements.'
      });
      setFeedback(data);
      setStatus('results');
    } catch (err: any) {
      toast.error(err.message || 'Analysis failed. Showing offline template.');
      setStatus('results');
    }
  };
                  ...
                  <Card className="premium-card border-emerald-500/20 bg-gradient-to-r from-emerald-500/5 to-teal-500/5">
                    ...
                    <CardContent className="space-y-6">
                      <div className="grid grid-cols-2 gap-x-8 gap-y-6">
                        {[
                          { label: 'Confidence', score: feedback.feedbackConfidence, color: 'bg-emerald-500' },
                          { label: 'Communication', score: feedback.feedbackCommunication, color: 'bg-blue-500' },
                          { label: 'Clarity', score: feedback.feedbackClarity, color: 'bg-indigo-500' },
                          { label: 'Tech Knowledge', score: feedback.feedbackTechKnowledge, color: 'bg-amber-500' },
                        ].map((metric, i) => (
                          <div key={i}>
                      ...
                      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur rounded-xl p-5 border shadow-sm space-y-4">
                        ...
                        <ul className="space-y-3 text-sm text-foreground/80">
                          {feedback.feedbackSuggestions.map((suggestion: string, idx: number) => (
