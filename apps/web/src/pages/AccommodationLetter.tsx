...
import { generateAccommodationLetter } from '@/lib/api/accommodations';
...
export const AccommodationLetter = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const data = await generateAccommodationLetter(disability, needs, additionalInfo);
      setLetterText(data.letterText);
      setGenerated(true);
    } catch (err: any) {
      toast.error(err.message || 'Letter generation failed. Using local template.');
      
      // Fallback local letter assembly
      const formattedNeeds = needs.map((n, i) => `${i + 1}. ${n}`).join('\n');
      const fallbackText = `To Whom It May Concern,
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl min-h-[80vh]">
      ...
      {!generated ? (
        <div className="space-y-6">
          ...
          <Button onClick={handleGenerate} className="w-full md:w-auto shadow-lg shadow-primary/20" disabled={isGenerating}>
            {isGenerating ? 'Generating Letter...' : 'Generate Letter'} <FileText className="w-4 h-4 ml-2" />
          </Button>
        </div>
      ) : (
        <motion.div
