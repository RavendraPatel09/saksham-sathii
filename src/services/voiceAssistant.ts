...
class VoiceAssistantService {
  ...
  // LOGGING
  // ────────────────────────────────────────────────────────────────
  // Debug-only logging: never runs in production builds, and never logs
  // spoken/heard content (which may contain sensitive personal data) —
  // see call sites below where transcript/text values are redacted.
  ...
  private buildRecognition() {
    ...
    rec.onresult = (event: any) => {
      ...
      this.log('Heard transcript (length):', transcript.length);
  ...
  public speak(text: string, onDone?: () => void) {
    ...
    this.log('Speaking (length):', text.length);
