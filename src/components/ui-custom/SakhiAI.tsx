            ...
            <AnimatePresence>
              {showMenu && (
                <motion.div
                  ...
                  className="mb-4 bg-white dark:bg-[#0F1726] border border-border/50 dark:border-white/[0.08] shadow-2xl rounded-2xl p-2 flex flex-col gap-1 w-48"
                >
        ...
        {isOpen && (
          ...
          >
            <Card className="premium-card flex flex-col h-full overflow-hidden shadow-2xl border-primary/20">
              ...
              <CardContent className="flex-1 overflow-y-auto p-4 bg-muted/10 space-y-4">
                {messages.map(msg => (
                  <div key={msg.id} className={`flex gap-2 max-w-[85%] ${msg.sender === 'user' ? 'ml-auto flex-row-reverse' : ''}`}>
                    ...
                    <div className={`p-3 rounded-2xl text-sm ${msg.sender === 'user' ? 'bg-primary text-primary-foreground rounded-tr-sm' : 'bg-white dark:bg-[#162033] border border-border/50 dark:border-white/[0.08] shadow-sm rounded-tl-sm'}`}>
                      {msg.text}
                    </div>
                  </div>
                ))}
                ...
                {isTyping && (
                  <div className="flex gap-2 max-w-[85%]">
                    ...
                    <div className="p-4 rounded-2xl bg-white dark:bg-[#162033] border border-border/50 dark:border-white/[0.08] shadow-sm rounded-tl-sm flex gap-1 items-center">
              ...
              <div className="p-3 bg-muted/20 border-t flex flex-wrap gap-2 overflow-x-auto no-scrollbar">
                {SUGGESTIONS.map(sug => (
                  <button 
                    ...
                    className="text-xs bg-white dark:bg-[#162033] border border-border/50 dark:border-white/[0.08] hover:border-primary text-muted-foreground hover:text-primary transition-colors px-3 py-1.5 rounded-full whitespace-nowrap shadow-sm"
                  >
                    {sug}
                  </button>
                ))}
              ...
              <CardFooter className="p-3 bg-white dark:bg-[#0F1726] border-t border-border/50 dark:border-white/[0.08]">
                <form 
