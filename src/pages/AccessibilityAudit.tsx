              ...
              {!prefs.reducedMotion && (
                <>
                  ...
                  <motion.div 
                    ...
                    className="absolute -top-8 -right-8 bg-white dark:bg-[#162033] p-2 rounded-full shadow-lg border border-border/50 dark:border-white/[0.08]"
                  >
      ...
      >
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <motion.div variants={itemVariants} className="md:col-span-2 space-y-6">
            <Card className="premium-card bg-white/90 dark:bg-[#0F1726]/90 overflow-hidden">
...
const ToggleRow = ({ id, label, checked, onChange }: { id: string, label: string, checked: boolean, onChange: () => void }) => (
  <motion.div variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0 } }}>
    <div className={`flex items-center justify-between p-5 rounded-xl border transition-all duration-300 ${checked ? 'bg-primary/5 border-primary/30 shadow-sm' : 'bg-white dark:bg-[#0F1726] border-border/50 dark:border-white/[0.08] hover:border-border hover:shadow-sm'}`}>
